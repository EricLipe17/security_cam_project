...
{
    if (!m_pOutParams->m_pFQP)
        free(m_pOutParams->m_pFQP);
...
int TranscodeContext::init_video_encoder(StreamContext* _pCtx, AVCodecContext* _pDecoderCodecCtx, AVRational framerate, StreamingParams params)
{
...
int TranscodeContext::init_audio_encoder(StreamContext* _pCtx, int sample_rate, StreamingParams params)
{
    ...
    if (!_pCtx->m_pAudioCodecCtx) {av_log(NULL, AV_LOG_ERROR, "could not allocate memory for codec context"); return -1;}
...
int TranscodeContext::fill_stream_info(AVStream* _pStream, const AVCodec** _ppCodec, AVCodecContext** _ppCodecCtx) {
    ...
    *_ppCodecCtx = avcodec_alloc_context3(*_ppCodec);
    if (!*_ppCodecCtx) {av_log(NULL, AV_LOG_ERROR, "failed to allocate memory for codec context"); return -1;}
...
int TranscodeContext::get_stream_refs(StreamContext* _pInCtx)
{
    for (int i = 0; i < _pInCtx->m_pFormatCtx->nb_streams; i++)
    {
        if (_pInCtx->m_pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            av_log(NULL, AV_LOG_WARNING, "skipping streams other than audio and video");
        }
    }
    return 0;
}

int TranscodeContext::init_decoder_stream_ctx(StreamContext** _ppInCtx, const AVFormatContext* _pDecodeCtx)
{
...
{
    avformat_alloc_output_context2(&m_pEncoderCtx->m_pFormatCtx, NULL, NULL, m_pOutParams->m_pFQN);
    AVRational framerate = av_guess_frame_rate(m_pDecoderCtx->m_pFormatCtx, m_pDecoderCtx->m_pVideoStream, NULL);\
...
int TranscodeContext::encode_video(StreamContext* _pDecodeCtx, StreamContext* _pEncodeCtx, AVFrame* _pInputFrame, AVPacket** _ppOutPkt_)
{
    ...
    while (response >= 0)
    {
        ...
        else if (response < 0)
        {
            ...
            av_log(NULL, AV_LOG_ERROR, "Error while receiving packet from _pEncodeCtx: %s", szResponse.c_str());
            return -1;
        }
...
int TranscodeContext::encode_audio(StreamContext* _pDecodeCtx, StreamContext* _pEncodeCtx, AVFrame* _pInputFrame, AVPacket** _ppOutPkt_)
{
...
TranscodeContext::TranscodeContext(StreamingParams* _pParams, AVFormatContext* _pDecodeCtx, char* _pFN)
{
    if (_pParams)
...
int TranscodeContext::transcode_audio(AVPacket* _pInputPkt, AVFrame* _pInputFrame, AVPacket** _ppOutPkt_)
{
    int response = avcodec_send_packet(m_pDecoderCtx->m_pAudioCodecCtx, _pInputPkt);
    if (response < 0) {
        std::string szResponse = get_error_msg(response);
        av_log(NULL, AV_LOG_ERROR, "Error while sending packet to _pDecodeCtx: %s", szResponse.c_str()); 
        return response;
    }
    ...
    if (!m_pOutParams->m_nRemuxVideo)
    {
        while (response >= 0) {
            ...
            if (response == AVERROR(EAGAIN) || response == AVERROR_EOF)
                ...
                std::string szResponse = get_error_msg(response);
                av_log(NULL, AV_LOG_ERROR, "Error while receiving frame from _pDecodeCtx: %s", szResponse.c_str());
                return response;
            }
...
int TranscodeContext::transcode_video(AVPacket* _pInputPkt, AVFrame* _pInputFrame, AVPacket** _ppOutPkt_)
{
...
AVFormatContext* TranscodeContext::GetDecoderFmtCtx()
{
    return m_pDecoderCtx->m_pFormatCtx;
}

AVRational TranscodeContext::GetSrcFrameRate()
{
    return m_nSrcFrameRate;
}

AVDictionary** TranscodeContext::GetMuxerOpts()
{
    return &m_pMuxer_Opts;
}